...
import { useGLTF, useAnimations, useHelper, useVideoTexture, Html } from "@react-three/drei";
import { Euler, GroupProps, useFrame } from "@react-three/fiber";
...
import { useSpring, animated, config, AnimatedComponent, SpringValue, SpringValues} from '@react-spring/three'
...
import { RectAreaLightSource, MonitorSpotLightSource, MailboxSpotLightSource } from '@/app/(home–components)/(room)/lights'; // Import RectAreaLightSource
...
export function RoomModel({ onMonitorClicked, onMailboxClicked, onBottleClicked, ...props }: RoomModelProps) {
  ...
  const mailbox = useRef<THREE.Mesh>(null!);

  const [coverCubeScale, setCoverCubeScale] = useState(17.648);
  // const [mailboxPosition, setMailboxPosition] = useState(new THREE.Vector3(5.322, 1.002, 10.081) as any);
  const mailboxPosition = group.current ? group.current.getObjectByName("Mailbox")?.position : new THREE.Vector3();
  ...
  const [mailboxSpringProps] = useSpring(() => ({
    mailboxScale: isMailboxHovered ? 1.1 : 1,
    config: {
      duration: 200
    },
  }), [isMailboxHovered]);
  ...
  useEffect(() => {
    const bounceInterval = setInterval(() => {
      arrowBounceRef.current += 1;
      setArrowBounce(Math.sin(arrowBounceRef.current * 0.2) * 0.2); // Adjust the values for desired bounce height and speed
  ...
  }, [animationsFinished, actions.FishAction]);
  ...
  useEffect(() => {
    ...
    mixer.addEventListener("finished", (event) => {
      actionsList.push(event.action.getClip().name);
  ...
  return (
    <>
      <RectAreaLightSource animationsFinished={animationsFinished}/>
      <animated.group ref={group} {...props} dispose={null}>
        <group name="Scene">
          ...
          <group name="Cover" position={[0.553, 12.167, -0.133]} scale={0}>
            <skinnedMesh
              name="Plane"
          ...
          <group name="Computer" position={[33.668, 5.176, 3.196]} scale={0}>
            ...
            {/* @ts-ignore */}
            <animated.mesh name='Computer' onClick={() => {setMonitorClick(!monitorClick); onMonitorClicked();}} onPointerOver={() => {if(!monitorClick){setIsMonitorHovered(true);}}} onPointerOut={() => {if(!monitorClick){setIsMonitorHovered(false);}}} scale={monitorSpringProps.monitorScale} position={monitorSpringProps.monitorPosition}>
              <mesh
                name="Cube014"
                // castShadow
                receiveShadow
              ...
              <Html position={[0, 2.4 + arrowBounce, 0.6]}
                style={{
                  ...
                  transform: `scale(${monitorClick ? 0 : 1})`
                }}
              >
                <svg xmlns="http://www.w3.org/2000/svg" height="24" viewBox="0 -960 960 960" width="24" >
                  <path d="M480-362q-8 0-15-2.5t-13-8.5L268-557q-11-11-11-28t11-28q11-11 28-11t28 11l156 156 156-156q11-11 28-11t28 11q11 11 11 28t-11 28L508-373q-6 6-13 8.5t-15 2.5Z"/>
          ...
          <group name="Mailbox" position={[5.322, 1.002, 10.081]} scale={0}>
            ...
            {/* @ts-ignore */}
            <animated.mesh ref={mailbox} name='Mailbox' onClick={() => {setMailboxClick(!mailboxClick); onMailboxClicked();}} onPointerOver={() => {if(!mailboxClick){setIsMailboxHovered(true)}}} onPointerOut={() => {if(!monitorClick){setIsMailboxHovered(false)}}} scale={mailboxSpringProps.mailboxScale}>
            <mesh
              name="Cylinder021"
              castShadow
              receiveShadow
          ...
          <animated.mesh
            ...
            position={monitorSpringProps.chairPosition}
            scale={0}
          />
          <animated.mesh
            name="Chair_legs"
            ...
            material={materials["Floor tile 2"]}
            position={monitorSpringProps.chairLegPosition}
            scale={0}
          />
          <mesh
            name="Clock"
          ...
          <mesh
            ...
            scale={0}
          >
            <LampLightSource intensity={lightOnSpringProps.lampLightIntensity} />
            ...
            <animated.meshStandardMaterial
              ...
              color={0xffffff}
              emissive={0xffffff}
              emissiveIntensity={lightOnSpringProps.lampLightIntensity}
              metalness={0}
              roughness={0.5}
              opacity={1}
            />
          ...
          <group name="Bottle group" onClick={() => {console.log('hello')}}>
